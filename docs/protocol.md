...
- `stream_id` is a 24-bit unsigned integer in the range `1..0xffffff`; `0` is invalid.
- The current protocol version is `KCPMUX_VERSION = 2`.
...
2. Looks up an existing connection by peer address.
3. If a connection already exists and is `CONNECTED`, treats the packet as a possible retransmission and resends a successful ACK.
4. Otherwise creates a passive connection and stores the peer extension data in `peer_proto_ext`.
5. Calls `engine->callbacks.conn_connect_notify`. Only `KCPMUX_ACK_RESULT_OK` accepts the connection; any other result rejects and closes it.
...
0                   1                   2                   4
+-------------------+-------------------+-------------------+-------------------+
| type = 0x82       | version           | result            | ext_len(u16)      |
+-------------------+-------------------+-------------------+-------------------+
| ext bytes ...                                                                 |
+-------------------------------------------------------------------------------+
```
...
The sender includes a `version` field, but the current receiver does not validate it for `CONN_CONNECT_ACK`; it only reads `result` and `ext_len`.

### Handshake Retransmission

While a connection is `CONNECTING`, `kcpmux_conn_update` checks every `ctrl_timeout_ms`. If no ACK was received and `retry_count < connect_retries`, it retransmits `CONN_CONNECT`; after the retry limit is reached, the connection enters `ERROR` and closes with `TIMEOUT`.
...
The receiver only requires the message to be at least 9 bytes and updates `last_recv_ts`. The current implementation does not use `time` or `seq` for RTT, replies, or deduplication.
...
- After exceeding `0xffffff`, allocation wraps to `1` or `2` according to local parity.

When receiving `STREAM_PAYLOAD` for an unknown `stream_id`, the protocol uses parity to decide whether the id could have been created by the peer. If the unknown id belongs to the local side's parity, the packet is rejected as invalid format.
...
4. Call `stream_create_notify`.
5. Feed the current payload into KCP.
...
3. The KCP output callback adds the 4-byte KCPMUX header to each KCP segment: `type(1) + stream_id(3)`.
...
3. It strips the 4-byte KCPMUX header and feeds the remaining bytes into that stream's KCP instance.
...
- The KCPMUX header is 4 bytes, so one wrapped KCP segment can carry at most 1496 bytes.
...
0                   1                   4
+-------------------+-------------------+-------------------+
| type = 0x93       | stream_id(u24)    | reason            |
+-------------------+-------------------+-------------------+
```
...
0                   1                   4
+-------------------+-------------------+-------------------+
| type = 0x94       | stream_id(u24)    | reason            |
+-------------------+-------------------+-------------------+
```
...
- Connection lookup is entirely based on the lower-layer peer address. Protocol messages do not carry a connection id.
...
- The `version` field in `CONN_CONNECT_ACK` is currently not validated by the receiver.
